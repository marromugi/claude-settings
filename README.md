...
yq は読み取り専用として扱う。別途 13 タスクで実測したところ、同じ問いに答えるのに
全文 Read の 1.1% (148,983 -> 1,699 tok)、`kubectl get -o yaml` のフィルタでは
32,900 -> 220 tok だった。一方 `yq -i` はインラインコメントの位置揃えを正規化し
`>` ブロックを 1 行に畳むため、触っていない行が diff に出る。書き込みは Edit で行う。
